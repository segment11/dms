...
(
    ...
    kafka_version              varchar(20),   -- 2.8.2
    config_template_id         int,
    config_overrides           varchar(2000), -- JSON, runtime dynamic broker config
    zk_connect_string          varchar(500),  -- host list without chroot
    zk_chroot                  varchar(200) not null, -- auto-generated /kafka/{name}, unique across services
    app_id                     int,
    port                       int,
    brokers                    int,
    ...
    extend_params              varchar(2000),
    broker_detail              varchar(4000), -- JSON, BrokerDetail
    last_updated_message       varchar(200),
    created_date               timestamp,
    updated_date               timestamp default current_timestamp
);
create unique index idx_km_service_name on km_service (name);
create unique index idx_km_service_zk_chroot on km_service (zk_chroot);
create index idx_km_service_config_template_id on km_service (config_template_id);
create index idx_km_service_status on km_service (status);
...
(
    ...
    name         varchar(50),
    des          varchar(200),
    config_items text,          -- JSON, server.properties overrides
    updated_date timestamp default current_timestamp
);
create unique index idx_km_config_template_name on km_config_template (name);
...
(
    ...
    config_overrides  varchar(2000), -- JSON, topic-level config
    status            varchar(20),   -- creating/active/deleting/deleted
    created_date      timestamp,
    updated_date      timestamp default current_timestamp
);
create index idx_km_topic_service_id on km_topic (service_id);
create unique index idx_km_topic_service_id_name on km_topic (service_id, name);
...
create index idx_km_job_busi_id on km_job (busi_id);

create table km_task_log
(
    id           int auto_increment primary key,
    job_id       int,
    step         varchar(100),
...
create index idx_km_task_log_job_id on km_task_log (job_id);
...
create index idx_km_snapshot_service_id on km_snapshot (service_id);
