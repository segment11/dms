...
class ClusterDnsAnswerHandler implements DmsDnsAnswerHandler {
    ...
    static final String CLUSTER_SUFFIX = 'cluster_'
    ...
    List<DnsRecord> answer2(String domain, int ttl) {
        ...
        if (arr.length >= 3) {
            def inner3 = arr[2]
            if (inner3.startsWith(CLUSTER_SUFFIX)) {
                clusterId = inner3[CLUSTER_SUFFIX.length()..-1] as int
            } else {
                return null
            }
        }
    ...
    List<DnsRecord> answer(DnsRecord dnsQuestion, int ttl) {
        ...
        if (arr.length >= 2) {
            def inner2 = arr[1]
            if (inner2.startsWith(CLUSTER_SUFFIX)) {
                clusterId = inner2[CLUSTER_SUFFIX.length()..-1] as int
            } else {
                return null
            }
        }
