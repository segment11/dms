...
class AgentHelper {
    ...
    private String lastUpdatedClusterHostsContent

    void refreshClusterHosts(int clusterId) {
        try {
            def f = new File('/etc/hosts')
            if (!f.exists() || !f.canRead() || !f.canWrite()) {
                log.warn 'cluster local hosts refresh fail: /etc/hosts can not read and write'
                return
            }
            String hostsContent = Agent.instance.get('/dms/api/cluster/hosts', [clusterId: clusterId as Object], String)
            if (!hostsContent) {
                return
            }

            if (lastUpdatedClusterHostsContent && lastUpdatedClusterHostsContent == hostsContent) {
                log.debug 'cluster local hosts refresh already, not changed'
                return
            }

            def oldLines = f.readLines().collect { it.trim() }.findAll { it && !it.startsWith('#') }
            List<Map<String, String>> newOnes = []
            hostsContent.readLines().collect { it.trim() }.findAll { it }.each {
                def arr = it.split(' ')
                newOnes << [ip: arr[0], host: arr[1], added: '']
            }
            ...
            oldLines.collect {
                def arr = it.split(/[ \t]/)
                def ip = arr[0].trim()
                def hosts = arr[1..-1].collect { it.trim() }.findAll { it }.join(' ')

                def newOne = newOnes.find { one -> one.get('host') == hosts }
                if (newOne) {
                    finalList << "${newOne.ip} ${hosts}".toString()
                    newOne.put('added', '1')
                } else {
                    finalList << "${ip} ${hosts}".toString()
                }
            }
            newOnes.findAll { '' == it.get('added') }.each { one ->
                finalList << "${one.ip} ${one.host}".toString()
            }
            f.text = finalList.join("\r\n")
            log.info 'cluster local hosts refresh done'
            log.info hostsContent

            lastUpdatedClusterHostsContent = hostsContent
        } catch (Exception e) {
            log.error('error refresh cluster hosts local', e)
        }
    }
}
